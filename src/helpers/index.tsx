...
import { MILK_ETH_LP_ADDRESSES } from "src/constants/addresses";
import { Environment } from "src/helpers/environment/Environment/Environment";
import { PairContract__factory } from "src/typechain";

/**
 * gets marketPrice from MILK-ETH v2
...
export async function getMarketValue(
  ...
  const lpContract = new ethers.Contract(
    MILK_ETH_LP_ADDRESSES[networkID as keyof typeof MILK_ETH_LP_ADDRESSES],
    PairContract__factory.abi,
    provider,
  );
  ...
  return {
    price: (Number(reserves[1].toString()) / Number(reserves[0].toString())) * ethPrice,
    totalLiquidity: Number(reserves[1].toString()) / 10 ** 18,
  };
}
 ...
 */
export async function getTokenPrice(tokenId = "weth"): Promise<number> {
  let tokenPrice = 1624;
  try {
...
export const isTestnet = (networkId: NetworkId) => {
  const testnets = [NetworkId.ARBITRUM_TESTNET, NetworkId.ARBITRUM_GOERLI, NetworkId.TESTNET_GOERLI];
